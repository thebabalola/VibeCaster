    ...
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-center">
          ...
          <p className="text-vibecaster-pink-light">Loading roast gallery...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-vibecaster-pink-light">
          Check out the funniest roasts from the VibeCaster community! Vote for your favorites! 🔥
        </p>
      </div>

      {/* Filter and Sort Controls */}
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-center">
        <div className="flex bg-vibecaster-purple-dark/20 rounded-lg p-1">
          {[
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-md transition-colors ${
                  filter === tab.id
                    ...
                    : "text-vibecaster-pink-light hover:text-white"
                }`}
              >
                <Icon size={16} />
        ...
        <div className="flex items-center gap-2">
          <FaSort className="text-vibecaster-pink-light" size={16} />
          <select
            value={sortBy}
            onChange={(e) => setSortBy(e.target.value)}
            className="bg-vibecaster-purple-dark/20 border border-vibecaster-lavender/20 rounded-lg px-3 py-2 text-white text-sm"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {roasts.map((roast) => (
          <div
            key={roast.id}
            className={`vibecaster-card p-4 transition-all duration-300 hover:shadow-lg hover:shadow-vibecaster-lavender/10 ${
              roast.isCurrentUser ? "ring-2 ring-vibecaster-lavender/50" : ""
            }`}
          >
            {/* User Info */}
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 bg-vibecaster-purple-dark rounded-full flex items-center justify-center">
                ...
                {roast.isCurrentUser && (
                  <span className="text-xs bg-vibecaster-lavender text-white px-2 py-1 rounded-full">
                    YOU
                  </span>
                )}
              </div>
              <span className="text-xs text-vibecaster-pink-light">{formatTimeAgo(roast.timestamp)}</span>
            </div>
              ...
              <div className="flex items-center gap-4">
                <button
                  ...
                  className={`flex items-center gap-1 transition-colors ${
                    roast.isLiked
                      ? "text-red-500"
                      : "text-vibecaster-pink-light hover:text-red-500"
                  }`}
                >
                  <FaHeart size={14} />
                  <span className="text-xs">{roast.likes}</span>
                </button>
                <button className="flex items-center gap-1 text-vibecaster-pink-light hover:text-white transition-colors">
                  <FaComment size={14} />
                  <span className="text-xs">{roast.comments}</span>
                </button>
              </div>
              <button className="text-vibecaster-pink-light hover:text-white transition-colors">
                <FaShare size={14} />
              </button>
            </div>
          </div>
      ...
      {/* Load More Button */}
      <div className="text-center">
        <button className="bg-vibecaster-lavender hover:bg-vibecaster-lavender/80 text-white px-6 py-3 rounded-lg transition-colors">
          Load More Roasts
        </button>
      </div>
    </div>
